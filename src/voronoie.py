...
# ------------------------------------------------------------------
def isEqual(a, b, relative_error=TOLERANCE):
...
class Context(object):
    def __init__(self, inputs=[]):
        ...
        self.edges = []  # edge 3-tuple: (line index, vertex 1 index, vertex 2 index)
        ...
        # custom
        self.bounds = []    # used to trim

    @property
    def inputs(self):
        return self._input_pts
    ...
    def outEdge(self, edge):
        ...
        if edge.ep[Edge.LE] is not None:
            site_num_l = edge.ep[Edge.LE].sitenum
        site_num_r = -1
        if edge.ep[Edge.RE] is not None:
            site_num_r = edge.ep[Edge.RE].sitenum
        ...
        self.polygons[edge.reg[0].sitenum].append((edge.edgenum, site_num_l, site_num_r))
        self.polygons[edge.reg[1].sitenum].append((edge.edgenum, site_num_l, site_num_r))
        self.edges.append((edge.edgenum, site_num_l, site_num_r))
        if not self.triangulate:
            if self.plot:
                self.clip_line(edge)
            elif self.doPrint:
                print("e {}\n {}\n {} " % edge.edgenum, site_num_l, site_num_r)

    @property
    def cell_polygons(self):
        """ Get the polygon paths for each cell
             l is the index of the line, v1 and v2 are
               the indices of the vertices at the end of the edge.  If
               v1 or v2 is -1, the line extends to infinity.
        """
        res = []
        for l, v1, v2 in self.edges:
            res.append([self.vertices[v1], self.vertices[v2]])
...
def voronoi(siteList, context):
    try:
        edge_list = EdgeList(siteList.xmin, siteList.xmax, len(siteList))
        priority_q = PriorityQueue(siteList.ymin, siteList.ymax, len(siteList))
        site_iter = siteList.iterator()

        bottom_site = site_iter.next()
        context.outSite(bottom_site)
        ...
        while True:
            ...
            if newsite and (priority_q.isEmpty or cmp(newsite, minpt) < 0):
                ...
                bot = lbnd.right_reg(bottom_site)
                ...
                bot = lbnd.left_reg(bottom_site)
                top = rbnd.right_reg(bottom_site)

                # output the triple of sites, stating that a circle goes through them
                mid = lbnd.right_reg(bottom_site)
...
# ------------------------------------------------------------------
class Site(object):
    def __init__(self, x=0.0, y=0.0, sitenum=0, weight=0.):
        self.x = x
        self.y = y
        self.weight = weight
    ...
    def __str__(self):
        return "Site #{} ({}, {})".format(self.sitenum, self.x, self.y)
...
class Edge(object):
    ...
    def __str__(self):
        return "(#{} a={}, b={}, c={}) ep {} reg {}".format(
            self.edgenum, self.a, self.b, self.c, self.ep, self.reg
        )
    ...
    def intersect(self, other):
        ...
        xint = (self.c * other.b - other.c * self.b) / d
        yint = (other.c * self.a - self.c * other.a) / d
        # create a new site at the point of intersection
        # this is a new vector event waiting to happen
        return Site(xint, yint)

    @staticmethod
    def bisect(s1, s2, r=0.5):
        newedge = Edge()
